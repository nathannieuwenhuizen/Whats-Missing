...
namespace Clayxels{
	...
	*/
	...
	public class ClayObject : MonoBehaviour{
		...
		/* Set this clayObject to any primitive currently defined inside claySDF.compute (box, sphere, etc.). */
		public void setPrimitiveType(int primType){
			this.primitiveType = primType;
		}

		/* Get the current primitive type with an id as defined inside claySDF.compute. */
		public int getPrimitiveType(){
			return this.primitiveType;
		}

		/* Set how this clayObject will blend with the previous one in the stack, 
		...
		/* Get the current blend value, result will range from -100.0 to 100.0 depending if it's a negative or positive blend. */
		public float getBlend(){
			return this.blend;
		}

		/* Set the color of this clayObject. */
		public void setColor(Color color){
			this.color = color;
		}

		/* Get the color of this clayObject. */
		public Color getColor(){
			return this.color;
		}

		/* Set an attribute on this primitive type as defined inside claySDF.compute. */
		...
		/* Get an attribute on this primitive type as defined inside claySDF.compute. */
		...
		/* Force this clayObject to update its internal solids list and notify the parent container. */
		...
		/* Get a specific solid inside this container.
		...
		/* How many solids are contained in this clayObject. 
			Depending if this clayObject was set to offset or spline mode, there might be multiple solids inside this clayObject. */
		public int getNumSolids(){
			return this.solids.Count;
		}

		/* If this clayObject was set to offset mode, 
		...
		/* If this clayObject was set to spline mode, 
		...
		/* Get the list of gameObjects used to control the spline if this was set to spline mode. */
		public List<GameObject> getSplinePoints(){
			return this.splinePoints;
		}

		/* Add a spline control point at the end of the list. */
		...
		/* Remove the last spline control point in the list. */
		...
		/* Which mode is currently set for this clayObject. */
		public ClayObjectMode getMode(){
			return this.mode;
		}

		/* Single mode: just one solid (box, sphere, etc.) that you can move around, scale and rotate like any other GameObject.
		...
		/* Get the parent clayContainer for this clayObject. */
